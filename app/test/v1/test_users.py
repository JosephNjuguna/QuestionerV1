...
#actual api endpoint test
    def test_1_good_sign_up(self):
        ...
        response = self.app.post(
            '/api/v1/auth/signup', data=json.dumps(self.user1), content_type='application/json')
        self.assertEqual(response.status_code, 201,msg='Successful registration')
    ...
    def test_5_not_found_email(self):
        ...
        response = self.app.post(
            '/api/v1/auth/login', data=json.dumps(self.user5), content_type='application/json')
        self.assertEqual(response.status_code, 404, msg="Email not found")

    def test_6_empty_key_field_login(self):
        """test empty key  """
        response = self.app.post(
            '/api/v1/auth/login', data=json.dumps(self.user3), content_type='application/json')
        self.assertEqual(response.status_code, 500, msg="Expecting a field key")

    def test_7_failed_log_in(self):
        """test failed log in due to incorrect email"""
        response = self.app.post(
            '/api/v1/auth/login', data=json.dumps(self.user6), content_type='application/json')
        self.assertEqual(response.status_code, 400, msg="Please Check Your Email")
